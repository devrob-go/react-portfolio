		...
		>
			<div className="z-10 transition-all duration-300 ease-in-out absolute sm:relative w-full top-0 max-w-screen-lg xl:max-w-screen-xl block sm:flex sm:justify-between sm:items-center py-6">
				...
				<div id='mobile-body-content'
					className={`fixed top-0 left-0 h-full w-full sm:w-80 pt-32 dark:bg-slate-900 bg-primary-light transition-all duration-500 ease-in-out ${showMenu ? 'visible mobile-body-content-visible' : 'mobile-body-content-hidden'
						}`}
				>
					<ul>
						{navLinks.map((link) => (
							<li>
								<NavLink
									...
									to={link.path}
									onClick={toggleMenu}
									className={`block text-center text-3xl font-extralight hover:text-secondary-dark dark:hover:text-ternary-light sm:mx-4 mb-3 sm:py-2 pt-3 sm:pt-2
